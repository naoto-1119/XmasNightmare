...
import com.xmsnm.xmasnightmare.ThirdPart2FragmentDirections
 ...
 */
class ThirdPart2Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_third_part2, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        view.findViewById<Button>(R.id.capture_button_santa_2).setOnClickListener {
            dispatchTakePictureIntent()
        }

        view.findViewById<Button>(R.id.next_button_6).setOnClickListener {
            findNavController().navigate(R.id.action_ThirdPart2Fragment_to_ThirdPart3Fragment)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger = truncate(accuracy[2] * 100) //changed to the corresponding element
            val accuracytoInt = accuracyInInteger.toInt()

            // Changes required to the corresponding element
            if (accuracy[2] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.santa_d4)
                view?.findViewById<TextView>(R.id.santa_d3)?.text = nextDialogue
//              changes image
                view?.findViewById<ImageView>(R.id.santa_image_2)
                    ?.setImageBitmap(imgAsBitmap)
//              hide lock image
                view?.findViewById<ImageView>(R.id.santa_image_lock)?.visibility = INVISIBLE
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button_santa_2)?.visibility = INVISIBLE
//              change defeated text to visible
                view?.findViewById<Button>(R.id.next_button_6)?.visibility = VISIBLE
...
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_heart, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_heart)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_heart)?.visibility = VISIBLE
            } else {
                val action =
                    ThirdPart2FragmentDirections.actionThirdPart2FragmentToGameoverFragment(
                        accuracytoInt
                    )
