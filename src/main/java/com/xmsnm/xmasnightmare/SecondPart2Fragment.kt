...
import com.xmsnm.xmasnightmare.SecondPart2FragmentDirections
 ...
 */
class SecondPart2Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_second_part2, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        view.findViewById<Button>(R.id.capture_button_raindeer_2).setOnClickListener {
            dispatchTakePictureIntent()
        }

        view.findViewById<Button>(R.id.next_button_3).setOnClickListener {
            findNavController().navigate(R.id.action_SecondPart2Fragment_to_SecondPart3Fragment)
        }
//        view.findViewById<Button>(R.id.next_round_button_2).setOnClickListener {
//            findNavController().navigate(R.id.action_SecondFragment_to_SecondPart2Fragment)
//        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger =
                truncate(accuracy[5] * 100) //changed to the corresponding element
            val accuracytoInt = accuracyInInteger.toInt()

            // Changes required to the corresponding element
            if (accuracy[5] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.raindeer_d4)
                view?.findViewById<TextView>(R.id.raindeer_d3)?.text = nextDialogue
//              changes hp
                view?.findViewById<TextView>(R.id.raindeer_hp_1)?.visibility = INVISIBLE
//              changes image
                view?.findViewById<ImageView>(R.id.raindeer_image_2)
                    ?.setImageBitmap(imgAsBitmap)
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button_raindeer_2)?.visibility = INVISIBLE
//              change defeated text to visible
                view?.findViewById<Button>(R.id.next_button_3)?.visibility = VISIBLE
...
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_mask, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_mask)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_mask)?.visibility = VISIBLE
            } else {
                val action =
                    SecondPart2FragmentDirections.actionSecondPart2FragmentToGameoverFragment(
                        accuracytoInt
                    )
