...
import com.xmsnm.xmasnightmare.FirstFragmentDirections
...
class FirstFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.next_button_0).setOnClickListener {
            findNavController().navigate(R.id.action_FirstFragment_to_FirstPart2Fragment)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger = truncate(accuracy[7] * 100)
            val accuracytoInt = accuracyInInteger.toInt()

            if (accuracy[7] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.ginger_bread_d2)
                view?.findViewById<TextView>(R.id.gbm_text_1)?.text = nextDialogue
//              changes hp
                view?.findViewById<TextView>(R.id.gbm_hp)?.visibility = INVISIBLE
//              changes image
                view?.findViewById<ImageView>(R.id.ginger_image)
                    ?.setImageBitmap(imgAsBitmap)
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button)?.visibility = INVISIBLE
//              change next round button to visible
                view?.findViewById<Button>(R.id.next_button_0)?.visibility = VISIBLE
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_punch, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_punch)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_punch)?.visibility = VISIBLE
            } else {
                val action =
                    FirstFragmentDirections.actionFirstFragmentToGameoverFragment(
                        accuracytoInt
                    )
    ...
    private fun tftest(bmp: Bitmap): FloatArray {56
