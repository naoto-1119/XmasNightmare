...
import com.xmsnm.xmasnightmare.ThirdFragmentDirections
...
class ThirdFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.capture_button_santa).setOnClickListener {
            dispatchTakePictureIntent()
        }

        view.findViewById<Button>(R.id.next_button_5).setOnClickListener {
            findNavController().navigate(R.id.action_ThirdFragment_to_ThirdPart2Fragment)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger = truncate(accuracy[3] * 100) //changed to the corresponding element
            val accuracytoInt = accuracyInInteger.toInt()

            // Changes required to the corresponding element
            if (accuracy[3] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.santa_d2)
                view?.findViewById<TextView>(R.id.santa_d1)?.text = nextDialogue
//              changes hp
                view?.findViewById<TextView>(R.id.santa_hp_3)?.visibility = INVISIBLE
//              changes image
                view?.findViewById<ImageView>(R.id.santa_image_1)
                    ?.setImageBitmap(imgAsBitmap)
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button_santa)?.visibility = INVISIBLE
//              change defeated text to visible
                view?.findViewById<Button>(R.id.next_button_5)?.visibility = VISIBLE
...
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_hide, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_hide)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_hide)?.visibility = VISIBLE
            } else {
                val action =
                    ThirdFragmentDirections.actionThirdFragmentToGameoverFragment(
                        accuracytoInt
                    )
