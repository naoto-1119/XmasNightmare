...
import com.xmsnm.xmasnightmare.ThirdPart3FragmentDirections
 ...
 */
class ThirdPart3Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_third_part3, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        view.findViewById<Button>(R.id.capture_button_santa_3).setOnClickListener {
            dispatchTakePictureIntent()
        }

        view.findViewById<Button>(R.id.next_button_7).setOnClickListener {
            findNavController().navigate(R.id.action_ThirdPart3Fragment_to_ThirdPart4Fragment)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger = truncate(accuracy[1] * 100) //changed to the corresponding element
            val accuracytoInt = accuracyInInteger.toInt()

            // Changes required to the corresponding element
            if (accuracy[1] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.santa_d6)
                view?.findViewById<TextView>(R.id.santa_d5)?.text = nextDialogue
//              changes image
                view?.findViewById<ImageView>(R.id.santa_image_potion)
                    ?.setImageBitmap(imgAsBitmap)
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button_santa_3)?.visibility = INVISIBLE
//              change defeated text to visible
                view?.findViewById<Button>(R.id.next_button_7)?.visibility = VISIBLE
...
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_drink, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_drink)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_drink)?.visibility = VISIBLE
            } else {
                val action =
                    ThirdPart3FragmentDirections.actionThirdPart3FragmentToGameoverFragment(
                        accuracytoInt
                    )
