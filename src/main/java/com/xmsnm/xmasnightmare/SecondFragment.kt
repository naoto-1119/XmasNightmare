...
import com.xmsnm.xmasnightmare.SecondFragmentDirections
...
class SecondFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.capture_button_raindeer).setOnClickListener {
            dispatchTakePictureIntent()
        }

        view.findViewById<Button>(R.id.next_button_2).setOnClickListener {
            findNavController().navigate(R.id.action_SecondFragment_to_secondPartHalfFragment)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            val accuracyInInteger = truncate(accuracy[6] * 100) //changed to the corresponding element
            val accuracytoInt = accuracyInInteger.toInt()

            // Changes required to the corresponding element
            if (accuracy[6] > 0.8) {
//              changes dialogue
                val nextDialogue = getString(R.string.raindeer_d1_5)
                view?.findViewById<TextView>(R.id.raindeer_d1)?.text = nextDialogue
//              changes hp
                view?.findViewById<TextView>(R.id.raindeer_hp_2)?.visibility = INVISIBLE
//              changes image
                view?.findViewById<ImageView>(R.id.raindeer_image)
                    ?.setImageBitmap(imgAsBitmap)
//              change take photo button to invisible
                view?.findViewById<Button>(R.id.capture_button_raindeer)?.visibility = INVISIBLE
//              change defeated text to visible
                view?.findViewById<Button>(R.id.next_button_2)?.visibility = VISIBLE
...
//              change text for punch accuracy
                val accuracyText = getString(R.string.accuracy_poke, accuracytoInt)
                view?.findViewById<TextView>(R.id.accuracy_poke)?.text = accuracyText
                view?.findViewById<TextView>(R.id.accuracy_poke)?.visibility = VISIBLE
            } else {
                val action =
                    SecondFragmentDirections.actionSecondFragmentToGameoverFragment(
                        accuracytoInt
                    )
